    ...
    | otherwise =
        let s = highestBit (x `div` 2) in
            (if isNothing s then s else
                (let j = fromJust s in Just (j * 2)))

highestBitHard :: Natural -> Maybe (Natural, Natural)
highestBitHard x
        | x == 0     = Nothing
        | x == 1     = Just (1, 0)
        | otherwise =
            let s = highestBitHard (x `div` 2) in
                (if isNothing s then s else
...
removeAtHard num (x : xs)
        | num < 0       = (Nothing, x : xs)
        | num == 0      = (Just x, xs)
        | otherwise     =
            let (maybeA, list) = removeAtHard (num - 1) xs in (maybeA, x : list)
...
stringSumHard = stringSumHard' []
        where
            ...
            stringSumHard' strToNum (x : xs)
                | isSpace x             =
                    if null strToNum
                        then stringSumHard' [] xs
                        else (let currInt = (read strToNum :: Int) in
                            let strSum = stringSumHard' [] xs in
                                if isNothing strSum
                                    then Nothing
                                    else Just (currInt + fromJust (stringSumHard' [] xs)))
                | x == '-' || x == '+' =
                    if null strToNum
                        then stringSumHard' [x] xs
                        else Nothing
                | not $ isDigit x       = Nothing
                | otherwise             = stringSumHard' (strToNum ++ [x]) xs
...
instance (Eq a) => Eq (Tree a) where
    ...
    (==) Leaf _         = False
    (==) _ Leaf         = False
    (==) (Node x left right) (Node y left1 right1) = x == y && left == left1 && right == right1
